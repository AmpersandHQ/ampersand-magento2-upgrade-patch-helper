...
class FunctionalTests extends \PHPUnit\Framework\TestCase
{
    ...
    public function testMagentoTwoFourNoDbHyva()
    {
        ...
        exec($this->generateAnalyseCommand('/dev/instances/../instances/magentom24nodb', '--hyva --pad-table-columns 130 --sort-by-type --vendor-namespaces Ampersand'), $output, $return);
        $this->assertEquals(0, $return, "The return code of the command was not zero");
        ...
        // We should get the same output regardless of whether we are connected to a DB or not
        $this->assertEquals($this->fileGetContents('/dev/phpunit/functional/expected_output/magentom24.hyva.out.txt'), $output);
    }
